...
function Shipper() {
  ...
  /* PAGINATION */
  const totalPages = Math.ceil(filteredShipper.length / itemsPerPage)
  const indexOfLastItem = currentPage * itemsPerPage
  const indexOfFirstItem = indexOfLastItem - itemsPerPage

  const currentShipper = filteredShipper.slice(
    indexOfFirstItem,
    indexOfLastItem
  )
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await addShipper(formData).unwrap()
      ...
      if (response?.data?.id) {
        navigate(`/editShipper/${response.data.id}`)
      }
      ...
      <div className={style.ListContainer}>
        ...
        <div className={style.pageHeaderContainer}>
          ...
          <div className={style.flexTitleHeader}>
            ...
            <h3 className={style.headerLaber}>Shipper</h3>
          </div>
            <p className={style.headerSubtitle}>View and manage shippers.</p>
          </div>
        ...
        <table>
          ...
          <tbody>
            {currentShipper.length === 0 ? (
              ...
              currentShipper.map((s) => (
                <tr key={s.id} className={style.bodyAffiliateTable}>
                  <td>{s.shipper}</td>
                  <td>
                    <button
                      className={style.editBtn}
                      onClick={() => navigate(`/editShipper/${s.id}`)}
                    >
                    <svg
