...
function EditCourier() {
  ...
  const {
    ...
    isError,
    error,
  } = useGetCourierByIdQuery(id)

  const [updateCourier, { isLoading: isUpdating }] =
    useUpdateCourierMutation()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateCourier({ id, ...formData }).unwrap()
      ...
      <div className={style.editCustomer}>
        <div className={style.EditflexTitleHeader}>
          <div className={style.flexheaderTitle}>
            ...
            <h3 className={style.headerLaber}>Update Courier</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className={style.editFormCustomer}>
          <label className={style.editLabel}>Courier:</label>
