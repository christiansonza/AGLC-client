...
function Broker() {
  ...
  const { data = [], isLoading, isError, error } = useGetBrokerQuery()
  const [addBroker] = usePostBrokerMutation()
  ...
  const filteredBroker = data.filter((b) =>
    b.broker.toLowerCase().includes(search.toLowerCase())
  )

  const totalPages = Math.ceil(filteredBroker.length / itemsPerPage)
  const indexOfLastItem = currentPage * itemsPerPage
  const indexOfFirstItem = indexOfLastItem - itemsPerPage

  const currentBroker = filteredBroker.slice(
    indexOfFirstItem,
    indexOfLastItem
  )
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await addBroker(formData).unwrap()
      ...
      if (response?.data?.id) {
        navigate(`/editBroker/${response.data.id}`)
      }
    } catch (err) {
        ...
        <div className={style.pageHeaderContainer}>
          <div className={style.flexTitleHeader}>
            <div className={style.flexheaderTitle}>
              ...
              <h3 className={style.headerLaber}>Broker</h3>
            </div>
        ...
        <table>
          ...
          <tbody>
            {currentBroker.length === 0 ? (
              ...
              currentBroker.map((b) => (
                <tr key={b.id} className={style.bodyAffiliateTable}>
                  <td>{b.broker}</td>

                  <td>
                    <button
                      className={style.editBtn}
                      onClick={() => navigate(`/editBroker/${b.id}`)}
                    >
                      <svg
