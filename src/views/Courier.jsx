...
function Courier() {
  ...
  const filteredCourier = data.filter((c) =>
    c.courier.toLowerCase().includes(search.toLowerCase())
  )

  const totalPages = Math.ceil(filteredCourier.length / itemsPerPage)
  const indexOfLastItem = currentPage * itemsPerPage
  const indexOfFirstItem = indexOfLastItem - itemsPerPage

  const currentCourier = filteredCourier.slice(
    indexOfFirstItem,
    indexOfLastItem
  )
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await addCourier(formData).unwrap()
      ...
      if (response?.data?.id) {
        navigate(`/editCourier/${response.data.id}`)
      }
    } catch (err) {
        ...
        <div className={style.pageHeaderContainer}>
          <div className={style.flexTitleHeader}>
            <div className={style.flexheaderTitle}>
              ...
              <h3 className={style.headerLaber}>Courier</h3>
            </div>

            <p className={style.headerSubtitle}>
              View and manage couriers.
            </p>
          </div>
          ...
          <div className={style.modalOverlay}>
            <div className={style.modal}>
              ...
              <form onSubmit={handleSubmit} className={style.formContainer}>
                <label className={style.modalLabel}>Courier:</label>
        ...
        <table>
          ...
          <tbody>
            {currentCourier.length === 0 ? (
              ...
              currentCourier.map((c) => (
                <tr key={c.id} className={style.bodyAffiliateTable}>
                  <td>{c.courier}</td>

                  <td>
                    <button
                      className={style.editBtn}
                      onClick={() => navigate(`/editCourier/${c.id}`)}
                    >
                      <svg
