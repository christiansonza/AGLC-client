...
function EditBroker() {
  ...
  const {
    ...
    isError,
    error,
  } = useGetBrokerByIdQuery(id)

  const [updateBroker, { isLoading: isUpdating }] =
    useUpdateBrokerMutation()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateBroker({ id, ...formData }).unwrap()
      ...
      <div className={style.editCustomer}>
        <div className={style.EditflexTitleHeader}>
          <div className={style.flexheaderTitle}>
            ...
            <h3 className={style.headerLaber}>Update Broker</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className={style.editFormCustomer}>
          <label className={style.editLabel}>Broker:</label>
